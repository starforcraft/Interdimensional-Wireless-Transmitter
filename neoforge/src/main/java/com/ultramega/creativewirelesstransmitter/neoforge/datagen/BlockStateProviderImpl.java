...
public class BlockStateProviderImpl extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        final ModelFile inactive = modelFile(createCreativeWirelessTransmitterIdentifier("block/creative_wireless_transmitter/inactive"));
        Blocks.INSTANCE.getCreativeWirelessTransmitter().forEach((color, id, block) -> {
            ...
            builder.forAllStates(blockState -> {
                ...
                if (Boolean.TRUE.equals(blockState.getValue(AbstractActiveColoredDirectionalBlock.ACTIVE))) {
                    model.modelFile(modelFile(createCreativeWirelessTransmitterIdentifier("block/creative_wireless_transmitter/" + color.getName())));
                } else {
